...
#define REP_DEFAULT             "796161"        // (REP_SKEET | REP_LEVEL | REP_CROWN | REP_DRAWCROWN | REP_HUNTERDP | REP_JOCKEYDP | REP_DEATHCHARGE | REP_CARALARM)
...
new                     IncapCount[2];
new                     FFCount[2];
new     Handle:         g_hCvarincapPenalty                               =INVALID_HANDLE;
new     Handle:         g_hCvarincapPenaltyMAX                            =INVALID_HANDLE;
new     Handle:         g_hCvarFFPenalty                                  =INVALID_HANDLE;
new     Handle:         g_hCvarFFPenaltyMAX                               =INVALID_HANDLE;
new     Handle:         g_hCvarSlevelBonus                                =INVALID_HANDLE;
new     Handle:         g_hCvarAPlevelBonus                               =INVALID_HANDLE;
new     Handle:         g_hCvarAlevelBonus                                =INVALID_HANDLE;
new     Handle:         g_hCvarBPlevelBonus                               =INVALID_HANDLE;
new     Handle:         g_hCvarBlevelBonus                                =INVALID_HANDLE;
new     Handle:         g_hCvarBMlevelBonus                               =INVALID_HANDLE;
new     Handle:         g_hCvarCPlevelBonus                               =INVALID_HANDLE;
new     Handle:         g_hCvarClevelBonus                                =INVALID_HANDLE;
new     Handle:         g_hCvarCMlevelBonus                                =INVALID_HANDLE;
new     Handle:         g_hCvarDlevelBonus                                =INVALID_HANDLE;
new     Handle:         g_hCvarElevelBonus                                =INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	g_hCvarincapPenalty = CreateConVar(   "sm_pbonus_PRIP",          "4",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	g_hCvarincapPenaltyMAX = CreateConVar(   "sm_pbonus_PRIPM",          "20",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	g_hCvarFFPenalty = CreateConVar(   "sm_pbonus_PRFFP",          "1",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	g_hCvarFFPenaltyMAX = CreateConVar(   "sm_pbonus_PRFFPM",          "10",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	g_hCvarSlevelBonus = CreateConVar(   "sm_pbonus_PRSL",          "2500",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	g_hCvarAPlevelBonus = CreateConVar(   "sm_pbonus_PRAPL",          "1000",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	g_hCvarAlevelBonus = CreateConVar(   "sm_pbonus_PRAL",          "250",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	g_hCvarBPlevelBonus = CreateConVar(   "sm_pbonus_PRBPL",          "100",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	g_hCvarBlevelBonus = CreateConVar(   "sm_pbonus_PRBL",          "25",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	g_hCvarBMlevelBonus = CreateConVar(   "sm_pbonus_PRBML",          "10",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	g_hCvarCPlevelBonus = CreateConVar(   "sm_pbonus_PRCPL",          "5",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	g_hCvarClevelBonus = CreateConVar(   "sm_pbonus_PRCL",          "0",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	g_hCvarCMlevelBonus = CreateConVar(   "sm_pbonus_PRCML",          "0",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	g_hCvarDlevelBonus = CreateConVar(   "sm_pbonus_PRDL",          "0",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	g_hCvarElevelBonus = CreateConVar(   "sm_pbonus_PREL",          "0",    "Penalty For the Tank Still alive when Survivors are in saferoom.", FCVAR_PLUGIN, true, 0.0);
	...
	RegConsoleCmd("sm_pr", Cmd_PR, "Prints the current PR bonus(es) for this round.");
...
public Action: Cmd_PR(client, args)
{
    if (!GetConVarBool(g_hCvarEnabled) || !GetConVarBool(g_hCvarDoDisplay)) { return Plugin_Continue; }
    
    DisplayPRBonus(client);
    return Plugin_Handled;
}
...
public Action: Cmd_GiveNades(client, args)
{
    if(GetClientTeam(client) == 2)
	{
	    if(PipeCooldown[client] < 0)
		{
	        ...
	        CPrintToChatAll("{olive}请注意{default}：玩家 {blue}%N{default} 使用了 {olive}%i{default} 分兑换了一枚{green}土雷{default}!", client, GetConVarInt(g_hCvarNadePenalty));
   ...
   if ( client > 0
		...
		&& IsTank(client))
	{
		if (bRefillHP[client] == false)
		{
		    ...
		    CPrintToChatAll("{olive}请注意{default}：Tank玩家 {red}%N{default} 使用了 {olive}%i{default} 分兑换了{green}%iHP{default}!", client, GetConVarInt(g_hCvarRefillPenalty), GetConVarInt(g_hCvarRefillAmount));
...
public Action:L4D2_OnEndVersusModeRound(bool:countSurvivors)
{
	...
	if (iSurvivalMultiplier > 0)
		{
		    PRbonus[RoundNum()] += GetPRThrowBonus() + GetPRMedBonus() + GetPRPillsBonus();
			if (PRbonus[RoundNum()] == 50)
			{
			    g_iBonus[RoundNum()] += GetConVarInt(g_hCvarSlevelBonus);
			    CreateTimer(5.5, PrintPRbonusS, _, TIMER_FLAG_NO_MAPCHANGE);
			}else if (PRbonus[RoundNum()] >= 45)
			{
			    g_iBonus[RoundNum()] += GetConVarInt(g_hCvarAPlevelBonus);
				CreateTimer(5.5, PrintPRbonusAP, _, TIMER_FLAG_NO_MAPCHANGE);
			}else if (PRbonus[RoundNum()] >= 40)
			{
			    g_iBonus[RoundNum()] += GetConVarInt(g_hCvarAlevelBonus);
				CreateTimer(5.5, PrintPRbonusA, _, TIMER_FLAG_NO_MAPCHANGE);
			}else if (PRbonus[RoundNum()] >= 35)
			{
			    g_iBonus[RoundNum()] += GetConVarInt(g_hCvarBPlevelBonus);
				CreateTimer(5.5, PrintPRbonusBP, _, TIMER_FLAG_NO_MAPCHANGE);
			}else if (PRbonus[RoundNum()] >= 30)
			{
			    g_iBonus[RoundNum()] += GetConVarInt(g_hCvarBlevelBonus);
				CreateTimer(5.5, PrintPRbonusB, _, TIMER_FLAG_NO_MAPCHANGE);
			}else if (PRbonus[RoundNum()] >= 25)
			{
			    g_iBonus[RoundNum()] += GetConVarInt(g_hCvarBMlevelBonus);
				CreateTimer(5.5, PrintPRbonusBM, _, TIMER_FLAG_NO_MAPCHANGE);
			}else if (PRbonus[RoundNum()] >= 20)
			{
			    g_iBonus[RoundNum()] += GetConVarInt(g_hCvarCPlevelBonus);
				CreateTimer(5.5, PrintPRbonusCP, _, TIMER_FLAG_NO_MAPCHANGE);
			}else if (PRbonus[RoundNum()] >= 15)
			{
			    g_iBonus[RoundNum()] += GetConVarInt(g_hCvarClevelBonus);
				CreateTimer(5.5, PrintPRbonusC, _, TIMER_FLAG_NO_MAPCHANGE);
			}else if (PRbonus[RoundNum()] >= 10)
			{
			    g_iBonus[RoundNum()] += GetConVarInt(g_hCvarCMlevelBonus);
				CreateTimer(5.5, PrintPRbonusCM, _, TIMER_FLAG_NO_MAPCHANGE);
			}else if (PRbonus[RoundNum()] >= 5)
			{
			    g_iBonus[RoundNum()] += GetConVarInt(g_hCvarDlevelBonus);
				CreateTimer(5.5, PrintPRbonusD, _, TIMER_FLAG_NO_MAPCHANGE);
			}else if (PRbonus[RoundNum()] >= 0)
			{
			    g_iBonus[RoundNum()] += GetConVarInt(g_hCvarElevelBonus);
				CreateTimer(5.5, PrintPRbonusE, _, TIMER_FLAG_NO_MAPCHANGE);
			}
		}else
		{
		    CreateTimer(5.5, PrintPRbonusF, _, TIMER_FLAG_NO_MAPCHANGE);
		}
    SetBonus();
}
...
public Action:PrintPRbonusS(Handle:timer) 
{
    new PRbonusThrow = GetPRThrowBonus();
	new PRbonusMed   = GetPRMedBonus();
	new PRbonusPills = GetPRPillsBonus();
	new PREBonus = GetConVarInt(g_hCvarSlevelBonus);
	new PRbonus2 = PRbonus[RoundNum()];
	PrintToChatAll("\x01生还者PerfectRound评价:\x03S\x01\n获得+\x05%i\x01分", PREBonus);
}

public Action:PrintPRbonusAP(Handle:timer) 
{
    new PRbonusThrow = GetPRThrowBonus();
	new PRbonusMed   = GetPRMedBonus();
	new PRbonusPills = GetPRPillsBonus();
	new PREBonus = GetConVarInt(g_hCvarAPlevelBonus);
	new PRbonus2 = PRbonus[RoundNum()];
	PrintToChatAll("\x01生还者PerfectRound评价:\x03A+\x01\n获得+\x05%i\x01分", PREBonus);
}

public Action:PrintPRbonusA(Handle:timer) 
{
    new PRbonusThrow = GetPRThrowBonus();
	new PRbonusMed   = GetPRMedBonus();
	new PRbonusPills = GetPRPillsBonus();
	new PREBonus = GetConVarInt(g_hCvarAlevelBonus);
	new PRbonus2 = PRbonus[RoundNum()];
	PrintToChatAll("\x01生还者PerfectRound评价:\x03A\x01\n获得+\x05%i\x01分", PREBonus);
}

public Action:PrintPRbonusBP(Handle:timer) 
{
    new PRbonusThrow = GetPRThrowBonus();
	new PRbonusMed   = GetPRMedBonus();
	new PRbonusPills = GetPRPillsBonus();
	new PREBonus = GetConVarInt(g_hCvarBPlevelBonus);
	new PRbonus2 = PRbonus[RoundNum()];
	PrintToChatAll("\x01生还者PerfectRound评价:\x03B+\x01\n获得+\x05%i\x01分", PREBonus);
}

public Action:PrintPRbonusB(Handle:timer) 
{
    new PRbonusThrow = GetPRThrowBonus();
	new PRbonusMed   = GetPRMedBonus();
	new PRbonusPills = GetPRPillsBonus();
	new PREBonus = GetConVarInt(g_hCvarBlevelBonus);
	new PRbonus2 = PRbonus[RoundNum()];
	PrintToChatAll("\x01生还者PerfectRound评价:\x03B\x01\n获得+\x05%i\x01分", PREBonus);
}

public Action:PrintPRbonusBM(Handle:timer) 
{
    new PRbonusThrow = GetPRThrowBonus();
	new PRbonusMed   = GetPRMedBonus();
	new PRbonusPills = GetPRPillsBonus();
	new PREBonus = GetConVarInt(g_hCvarBMlevelBonus);
	new PRbonus2 = PRbonus[RoundNum()];
	PrintToChatAll("\x01生还者PerfectRound评价:\x03B-\x01\n获得+\x05%i\x01分", PREBonus);
}

public Action:PrintPRbonusCP(Handle:timer) 
{
    new PRbonusThrow = GetPRThrowBonus();
	new PRbonusMed   = GetPRMedBonus();
	new PRbonusPills = GetPRPillsBonus();
	...
	if(PREBonus > 0)
	{
	    PrintToChatAll("\x01生还者PerfectRound评价:\x03C+\x01\n获得+\x05%i\x01分", PREBonus);
	}
	else
	{
	    PrintToChatAll("\x01生还者PerfectRound评价:\x03C+\x01 \n没有得分，期待下一次的完美轮", PRbonus2);
	}
}

public Action:PrintPRbonusC(Handle:timer) 
{
    new PRbonusThrow = GetPRThrowBonus();
	new PRbonusMed   = GetPRMedBonus();
	new PRbonusPills = GetPRPillsBonus();
	new PREBonus = GetConVarInt(g_hCvarClevelBonus);
	new PRbonus2 = PRbonus[RoundNum()];
	if(PREBonus > 0)
	{
	    PrintToChatAll("\x01生还者PerfectRound评价:\x03C\x01\n获得+\x05%i\x01分", PREBonus);
...
public Action:PrintPRbonusCM(Handle:timer) 
{
    new PRbonusThrow = GetPRThrowBonus();
	new PRbonusMed   = GetPRMedBonus();
	new PRbonusPills = GetPRPillsBonus();
	new PREBonus = GetConVarInt(g_hCvarCMlevelBonus);
	new PRbonus2 = PRbonus[RoundNum()];
	if(PREBonus > 0)
	{
	    PrintToChatAll("\x01生还者PerfectRound评价:\x03C-\x01\n获得+\x05%i\x01分", PREBonus);
...
public Action:PrintPRbonusD(Handle:timer) 
{
    new PRbonusThrow = GetPRThrowBonus();
	new PRbonusMed   = GetPRMedBonus();
	new PRbonusPills = GetPRPillsBonus();
	new PREBonus = GetConVarInt(g_hCvarDlevelBonus);
	new PRbonus2 = PRbonus[RoundNum()];
	if(PREBonus > 0)
	{
	    PrintToChatAll("\x01生还者PerfectRound评价:\x03D\x01\n获得+\x05%i\x01分", PREBonus);
	}
	else
	{
	    PrintToChatAll("\x01生还者PerfectRound评价:\x03D\x01 \n没有得分，期待下一次的完美轮", PRbonus2);
...
public Action:PrintPRbonusE(Handle:timer) 
{
    new PRbonusThrow = GetPRThrowBonus();
	new PRbonusMed   = GetPRMedBonus();
	new PRbonusPills = GetPRPillsBonus();
	...
	if(PREBonus > 0)
	{
	    PrintToChatAll("\x01生还者PerfectRound评价:\x03E\x01\n获得+\x05%i\x01分", PREBonus);
	}
	else
	{
	    PrintToChatAll("\x01生还者PerfectRound评价:\x03E\x01 \n没有得分，期待下一次的完美轮", PRbonus2);
...
public Action:PrintPRbonusF(Handle:timer) 
{
	PrintToChatAll("\x01生还者PerfectRound评价:\x03F\x01 \n期待队伍的下一次完美轮进屋");
}
...
stock DisplayBonus(client=-1)
{
    ...
    for (new round = 0; round <= RoundNum(); round++)
    {
        if (g_bRoundOver[round]) {
            Format(msgPartHdr, sizeof(msgPartHdr), "轮数 \x05%i\x01 额外得分", round+1);
        } else {
            Format(msgPartHdr, sizeof(msgPartHdr), "当前额外得分");
        }
...
stock ReportChange(bonusChange, client=-1, absoluteSet=false)
{
    ...
    if (absoluteSet) {  // set to a specific value
        Format(msgPartBon, sizeof(msgPartBon), "额外得分设置为: \x04%i\x01", g_iBonus[RoundNum()]);
    } else {
        Format(msgPartBon, sizeof(msgPartBon), "额外得分变化: %s\x04%i\x01",
...
stock DisplayPRBonus(client=-1)
{
	...
	PRbonus1 = PRbonusThrow + PRbonusMed + PRbonusPills + PRbonus[RoundNum()];


    if (PRbonus1 == 50)
	{
	    Format(sSurvivorState[RoundNum()], 32, "S");
	}else if (PRbonus1 >= 45)
	{
		Format(sSurvivorState[RoundNum()], 32, "A+");
	}else if (PRbonus1 >= 40)
	{
		Format(sSurvivorState[RoundNum()], 32, "A");
	}else if (PRbonus1 >= 35)
	{
		Format(sSurvivorState[RoundNum()], 32, "B+");
	}else if (PRbonus1 >= 30)
	{
		Format(sSurvivorState[RoundNum()], 32, "B");
	}else if (PRbonus1 >= 25)
	...
	{
		Format(sSurvivorState[RoundNum()], 32, "C+");
	}else if (PRbonus1 >= 15)
	{
		Format(sSurvivorState[RoundNum()], 32, "C");
	}else if (PRbonus1 >= 10)
	...
	{
		Format(sSurvivorState[RoundNum()], 32, "D");
	}else if (PRbonus1 >= 0)
	{
		Format(sSurvivorState[RoundNum()], 32, "E");
	}
        
    PrintToChat(client, "目前的PerfectRound得分为：%i/50, 评价:%s", PRbonus1, sSurvivorState[RoundNum()]);
}
...
public Action: Event_IncapStart( Handle:event, const String:name[], bool:dontBroadcast )
{
    ...
    new IPMax = GetConVarInt(g_hCvarincapPenaltyMAX);
    
	if(GetClientTeam(client) == 2 && IncapCount[RoundNum()] <= IPMax && PRbonus[RoundNum()] > 0)
	{
	    PRbonus[RoundNum()] -= GetConVarInt(g_hCvarincapPenalty);
		IncapCount[RoundNum()] += 1;
		//PrintToChatAll("生还者倒地，PR分降低%i", GetConVarInt(g_hCvarincapPenalty));
	}
...
public Action: Event_FriendlyFire( Handle:event, const String:name[], bool:dontBroadcast )
{
	...
	new FFMax = GetConVarInt(g_hCvarFFPenaltyMAX);
    
	if(GetClientTeam(victim) == 2 && FFCount[RoundNum()] <= FFMax && PRbonus[RoundNum()] > 0)
	{
	    PRbonus[RoundNum()] -= GetConVarInt(g_hCvarFFPenalty);
		FFCount[RoundNum()] += 1;
		//PrintToChatAll("生还者黑枪，PR分降低%i", GetConVarInt(g_hCvarFFPenalty));
	}
...
stock HandleDeadstop( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        ...
        {
            CPrintToChatAll( "{green}★ {olive}%N {blue}推停了 {olive}一只Hunter", attacker );
        }
    }
...
stock HandleSkeet( attacker, victim, bool:bMelee = false, bool:bSniper = false, bool:bGL = false )
{
    if ( GetConVarBool(g_hCvarReport))
    {
        ...
        else if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            PrintToChatAll( "\x04%N\x01 %s空爆了 \x05%N\x01.",
				...
				if (bMelee)
				{
					...
					CPrintToChatAll( "生还者队因为这个操作获得了{red}%i{default}分", RoundToFloor(GetConVarFloat(g_hCvarMeleeSkillBonus)));
				} else if (bSniper)
				{
				    g_iBonus[RoundNum()] += RoundToFloor(GetConVarFloat(g_hCvarSniperSkillBonus));
					sniperskeetcount[attacker] += 1;
					CPrintToChatAll( "生还者队因为这个操作获得了{red}%i{default}分", RoundToFloor(GetConVarFloat(g_hCvarSniperSkillBonus)));
				}
				else
				{
				g_iBonus[RoundNum()] += RoundToFloor(GetConVarFloat(g_hCvarNormalSkillBonus));
				CPrintToChatAll( "生还者队因为这个操作获得了{red}%i{default}分", RoundToFloor(GetConVarFloat(g_hCvarNormalSkillBonus)));
				}
				...
				CPrintToChatAll( "{green}什么叫做自信啊{default}\n生还者队因为 {olive}%N{default} 的{olive}双连近战{default}空爆获得了{red}%i{default}分!!", attacker, RoundToFloor(GetConVarFloat(g_hCvarDoubleSkeet) * GetConVarFloat(g_hCvarMultiMeleeBonus)));
				...
				CPrintToChatAll( "{green}这才是我的实力{default}\n生还者队因为 {olive}%N{default} 的{olive}三连近战{default}空爆获得了{red}%i{default}分!!!!!", attacker, RoundToFloor(GetConVarFloat(g_hCvarTripleSkeet) * GetConVarFloat(g_hCvarMultiMeleeBonus)));
				...
				CPrintToChatAll( "{green}太菜了，太菜啦{default}\n生还者队因为 {olive}%N{default} 的{olive}四连近战{default}空爆获得了{red}%i{default}分!!!!!!!", attacker, RoundToFloor(GetConVarFloat(g_hCvarQuadSkeet) * GetConVarFloat(g_hCvarMultiMeleeBonus)));
				...
				CPrintToChatAll( "{green}什么叫做自信啊{default}\n生还者队因为 {olive}%N{default} 的{olive}双连爆头{default}空爆获得了{red}%i{default}分!!", attacker, RoundToFloor(GetConVarFloat(g_hCvarDoubleSkeet) * GetConVarFloat(g_hCvarMultiSniperBonus)));
				...
				CPrintToChatAll( "{green}这才是我的实力{default}\n生还者队因为 {olive}%N{default} 的{olive}三连爆头{default}空爆获得了{red}%i{default}分!!!!!", attacker, RoundToFloor(GetConVarFloat(g_hCvarTripleSkeet) * GetConVarFloat(g_hCvarMultiSniperBonus)));
				...
				CPrintToChatAll( "{green}太菜了，太菜啦{default}\n生还者队因为 {olive}%N{default} 的{olive}四连爆头{default}空爆获得了{red}%i{default}分!!!!!!!", attacker, RoundToFloor(GetConVarFloat(g_hCvarQuadSkeet) * GetConVarFloat(g_hCvarMultiSniperBonus)));
				...
				CPrintToChatAll( "生还者队因为 {olive}%N{default} 的{olive}双连{default}空爆获得了{red}%i{default}分", attacker, RoundToFloor(GetConVarFloat(g_hCvarDoubleSkeet)));
				} else if(skeetcount[attacker] == 3)
				{
				g_iBonus[RoundNum()] += RoundToFloor(GetConVarFloat(g_hCvarTripleSkeet));
				CPrintToChatAll( "生还者队因为 {olive}%N{default} 的{olive}三连{default}空爆获得了{red}%i{default}分!", attacker, RoundToFloor(GetConVarFloat(g_hCvarTripleSkeet)));
				} else if(skeetcount[attacker] == 4)
				{
				g_iBonus[RoundNum()] += RoundToFloor(GetConVarFloat(g_hCvarQuadSkeet));
				CPrintToChatAll( "生还者队因为 {olive}%N{default} 的{olive}四连{default}空爆获得了{red}%i{default}分!!!", attacker, RoundToFloor(GetConVarFloat(g_hCvarQuadSkeet)));
        ...
        {
            PrintToChatAll( "\x04%N\x01 %s空爆了一个Hunter.",
                    attacker,
                    (bMelee) ? "近战": ((bSniper) ? "爆头" : ((bGL) ? "榴弹" : "") )
                );
        }
...
HandleRockSkeeted( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        ...
        */
        CPrintToChatAll( "{green}★ {olive}%N {blue}爆掉了 {default}一个石头", attacker );
    }
...
stock HandleHunterDP( attacker, victim, actualDamage, Float:calculatedDamage, Float:height, bool:playerIncapped = false )
{
    ...
    if (GetConVarBool(g_hCvarReport)
        ...
        {
            CPrintToChatAll( "{green}★★ {olive}%N {red}高扑了 {olive}%N {default}(高度: {red}%i{default})", attacker,  victim, RoundFloat(height) );
        }
	...
	CPrintToChatAll( "生还者队因为这个高扑损失了{red}%i{default}分", RoundToFloor(height / 20 * GetConVarFloat(g_hCvarHPtimes)));
    }
	if (    height >= 1200
        
    ) {
        g_iBonus[RoundNum()] -= RoundToFloor(height / 20 * GetConVarInt(g_hCvarHPtimes) * 0.5);
	CPrintToChatAll( "高扑距离超过1200，额外损失{red}%i{default}分", RoundToFloor(height / 20 * GetConVarFloat(g_hCvarHPtimes) * 0.5));
    }
	if (    height >= 2000
        
    ) {
        g_iBonus[RoundNum()] -= RoundToFloor(height / 20 * GetConVarInt(g_hCvarHPtimes));
	CPrintToChatAll( "高扑距离超过2000，额外损失{red}%i{default}分", RoundToFloor(height / 20 * GetConVarFloat(g_hCvarHPtimes)));
    }
	if (    height >= 2400
        
    ) {
        g_iBonus[RoundNum()] -= RoundToFloor(height / 20 * GetConVarInt(g_hCvarHPtimes) * 1.5);
	CPrintToChatAll( "高扑距离超过2400，额外损失{red}%i{default}分", RoundToFloor(height / 20 * GetConVarFloat(g_hCvarHPtimes) * 1.5));
    }
}
    ...
    if ( attacker != pinVictim && GetConVarBool(g_hCvarReport))
    {
        ...
        if ( fClearTime != -1.0 && fClearTime <= fMinTime )
        {
            if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
            {
                ...
                {
                    CPrintToChatAll( "{green}★ {olive}%N 从 {olive}%N{default}的%s手中 {blue}快速救下了 {olive}%N {default} ({blue}%.2f {default}秒)",
                            attacker, victim,
                            g_csSIClassName[zombieClass], pinVictim,
                            fClearTime
                        );
                } else {
                    CPrintToChatAll( "{green}★ {olive}%N 从 {olive}%N{default}的%s手中 {blue}快速救下了队友 ({blue}%.2f {default}秒)",
                            attacker, victim,
            ...
            else if ( IS_VALID_INGAME(attacker) )
            {
                if ( IS_VALID_INGAME(pinVictim) )
                {
                    CPrintToChatAll( "{green}★ {olive}%N 从 {olive}%s手中 {blue}快速救下了 {olive}%N {default} ({blue}%.2f {default}秒)",
                            attacker,
                            g_csSIClassName[zombieClass], pinVictim,
                            fClearTime
                        );
                } else {
                    CPrintToChatAll( "{green}★ {olive}%N 从 {olive}%s手中 {blue}快速救下了队友{default} ({blue}%.2f {default}秒)",
                            attacker,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
...
stock HandleBHopStreak( survivor, streak, Float: maxVelocity )
{
    if (    GetConVarBool(g_hCvarReport) &&
        ...
        CPrintToChatAll("{green}★ {olive}%N {default}在一次{olive}BunnyHop中做出了 {blue}%i{default} 个连跳 ({blue}最高速度: {olive}%.1f{default})",
		        survivor,
                streak,
                maxVelocity
            );
