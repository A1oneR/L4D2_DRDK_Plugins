...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_status", Status_Cmd, "List the Status of all players in game");
	RegConsoleCmd("sm_pings", Pings_Cmd, "List the Pings of all players in game");
...
{
	ReplyToCommand(client, "[!] Status list:");
	if (args < 1)
	{
		int ID;
		char tmp_steamid[21];
		...
		for (int i=1; i<=MaxClients; i++)
		{
			...
			GetClientAuthId(i, AuthId_Steam2, tmp_steamid, sizeof(tmp_steamid));		
			GetClientName(i, tmp_name, 35);
			
			ReplyToCommand(client, "#%-4.4d %-32.32s %-20.20s", ID, tmp_name, tmp_steamid);
...
public Action Pings_Cmd(int client, int args)
{
	ReplyToCommand(client, "[!] Pings list:");
	for (int i=1; i<=MaxClients; i++)
	{
		if (!IsValidClient(i))
		{
		...
		int targetPing = RoundToNearest(GetClientLatency(i, NetFlow_Outgoing) * 1024);
		CReplyToCommand(client, "{GREEN}[SM]{DEFAULT} {LIGHTGREEN}%N{DEFAULT}'s ping is {GREEN}%d{DEFAULT}!", i, targetPing);
	}
	return Plugin_Handled;
}
