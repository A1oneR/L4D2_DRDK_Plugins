...
public OnConfigsExecuted()
{
	...
	iPillWorth = CLAMP(RoundToNearest(GetConVarFloat(hCvarPillsBonusFactor) * (fPermHpWorth / GetConVarFloat(hCvarPillsHpFactor))), 1, GetConVarInt(hCvarPillsMaxBonus)); // make it pretty
        iMedWorth = CLAMP(RoundToNearest(GetConVarFloat(hCvarMedBonusFactor) * (fPermHpWorth / GetConVarFloat(hCvarMedHpFactor))), 1, GetConVarInt(hCvarMedMaxBonus)); // make it pretty#if SM2_DEBUG
...
public Action:CmdBonus(client, args)
{
	...
	if (StrEqual(sCmdType, "full"))
	{
		...
		{
			PrintToChat(client, "%s\x01R\x04#1\x01 得分: \x05%d\x01/\x05%d\x01 <\x03%.1f%%\x01> [%s]", PLUGIN_TAG, RoundToFloor(fSurvivorBonus[0]), RoundToFloor(fMapBonus + fMaxPillsBonus), RoundToFloor(fMapBonus + fMaxMedBonus), CalculateBonusPercent(fSurvivorBonus[0]), sSurvivorState[0]);
		}
		PrintToChat(client, "%s\x01R\x04#%i\x01 得分: \x05%d\x01 <\x03%.1f%%\x01> [血量: \x05%d\x01 <\x03%.1f%%\x01> | 伤害: \x05%d\x01 <\x03%.1f%%\x01> | 药: \x05%d\x01 <\x03%.1f%%\x01> | 包: \x05%d\x01 <\x03%.1f%%\x01>]", PLUGIN_TAG, InSecondHalfOfRound() + 1, RoundToFloor(fHealthBonus + fDamageBonus + fPillsBonus + fMedBonus), CalculateBonusPercent(fHealthBonus + fDamageBonus + fPillsBonus + fMedBonus, fMapHealthBonus + fMapDamageBonus + fMaxPillsBonus + fMaxMedBonus), RoundToFloor(fHealthBonus), CalculateBonusPercent(fHealthBonus, fMapHealthBonus), RoundToFloor(fDamageBonus), CalculateBonusPercent(fDamageBonus, fMapDamageBonus), RoundToFloor(fPillsBonus), CalculateBonusPercent(fPillsBonus, fMaxPillsBonus), RoundToFloor(fMedBonus), CalculateBonusPercent(fMedBonus, fMaxMedBonus));
		// R#1 Bonus: 556 <69.5%> [HB: 439 <73.1%> | DB: 117 <58.5%> | Pills: 90 <75.0%>]
	}
	else if (StrEqual(sCmdType, "lite"))
	{
		PrintToChat(client, "%s\x01R\x04#%i\x01 得分: \x05%d\x01 <\x03%.1f%%\x01>", PLUGIN_TAG, InSecondHalfOfRound() + 1, RoundToFloor(fHealthBonus + fDamageBonus + fPillsBonus + fMedBonus), CalculateBonusPercent(fHealthBonus + fDamageBonus + fPillsBonus + fMedBonus, fMapHealthBonus + fMapDamageBonus + fMaxPillsBonus + fMaxMedBonus));
		// R#1 Bonus: 556 <69.5%>
	}
	else
	{
		...
		PrintToChat(client, "%s\x01R\x04#%i\x01 得分: \x05%d\x01 <\x03%.1f%%\x01> [血量: \x03%.0f%%\x01 | 伤害: \x03%.0f%%\x01 | 药: \x03%.0f%%\x01 | 包: \x03%.0f%%\x01]", PLUGIN_TAG, InSecondHalfOfRound() + 1, RoundToFloor(fHealthBonus + fDamageBonus + fPillsBonus + fMedBonus), CalculateBonusPercent(fHealthBonus + fDamageBonus + fPillsBonus + fMedBonus, fMapHealthBonus + fMapDamageBonus + fMaxPillsBonus + fMaxMedBonus), CalculateBonusPercent(fHealthBonus, fMapHealthBonus), CalculateBonusPercent(fDamageBonus, fMapDamageBonus), CalculateBonusPercent(fPillsBonus, fMaxPillsBonus), CalculateBonusPercent(fMedBonus, fMaxMedBonus));
...
public Action:L4D2_OnEndVersusModeRound(bool:countSurvivors)
{
	...
	fSurvivorBonus[team] = GetSurvivorHealthBonus() + GetSurvivorDamageBonus() + GetSurvivorPillBonus() + GetSurvivorMedBonus();
...
public Action:PrintRoundEndStats(Handle:timer) 
{
	...
	{
		PrintToChatAll("%s\x01本局轮数 \x04%i\x01 得分: \x05%d\x01/\x05%d\x01 <\x03%.1f%%\x01> [%s]", PLUGIN_TAG, (i + 1), RoundToFloor(fSurvivorBonus[i]), RoundToFloor(fMapBonus + float(iPillWorth * iTeamSize) + float(iMedWorth * iTeamSize)), CalculateBonusPercent(fSurvivorBonus[i]), sSurvivorState[i]);
