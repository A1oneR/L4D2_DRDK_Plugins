...
#define PLUGIN_VERSION "1.1 AirServer"
...
#define CUSTOM_MODEL_PATH_SIZE 6	

//Z:***ADD NEW MODEL PATHS HERE!!!***
static const char sCustomPaths[CUSTOM_MODEL_PATH_SIZE][] =
{
	"models/AirServer/Amiya.mdl",
	"models/AirServer/AN94.mdl",
	"models/AirServer/Ansel.mdl",
	"models/AirServer/Lappland.mdl",
	"models/AirServer/Red.mdl",
	"models/AirServer/Ump40.mdl"
};
//Z:***ADD NEW ENUM HERE!!!***
...
public Plugin myinfo =
{
	name = PLUGIN_NAME,
	author = "Lux, A1R",
	description = "Allows humans to choose LMC custom model with cookiesaving",
...
public void OnPluginStart()
{
	...
	hCvar_ThirdPersonTime = CreateConVar("lmc_thirdpersontime", "5.0", "How long (in seconds) the client will be in thirdperson view after selecting a model from !lmc command. (0.5 < = off)", FCVAR_NOTIFY, true, 0.0, true, 360.0);
...
public void OnMapStart()
{
	...
	if(bPrecacheModels)
	{
		...
		*/
		for(i = 0; i < CUSTOM_MODEL_PATH_SIZE; i++)
			PrecacheModel(sCustomPaths[i], true);
	}
...
public Action ShowMenu(int iClient, int iArgs)
{
	...
	AddMenuItem(hMenu, "26", "Amiya 阿米娅");
	AddMenuItem(hMenu, "27", "AN94");
	AddMenuItem(hMenu, "28", "Ansel 安赛尔");
	AddMenuItem(hMenu, "29", "Lappland 拉普兰德");
	AddMenuItem(hMenu, "30", "Red 红");
	AddMenuItem(hMenu, "31", "UMP40");
...
void ModelIndex(int iClient, int iCaseNum, bool bUsingMenu=false)
{
	...
	switch(iCaseNum)
	{
		...
		case 26:
		{
			if(IsModelValid(iClient, LMCModelSectionType_Custom, view_as<int>(MODEL_AMIYA)))
				LMC_L4D2_SetTransmit(iClient, LMC_SetClientOverlayModel(iClient, sCustomPaths[MODEL_AMIYA]));

			if(!bUsingMenu && !bAutoApplyMsg[iClient])
				return;

			PrintToChat(iClient, "\x04[LMC] \x03Model is \x04Amiya");
		...
		case 27:
		{
			if(IsModelValid(iClient, LMCModelSectionType_Custom, view_as<int>(MODEL_AN94)))
				LMC_L4D2_SetTransmit(iClient, LMC_SetClientOverlayModel(iClient, sCustomPaths[MODEL_AN94]));

			if(!bUsingMenu && !bAutoApplyMsg[iClient])
				return;

			PrintToChat(iClient, "\x04[LMC] \x03Model is \x04AN94");
		...
		case 28:
		{
			if(IsModelValid(iClient, LMCModelSectionType_Custom, view_as<int>(MODEL_ANSEL)))
				LMC_L4D2_SetTransmit(iClient, LMC_SetClientOverlayModel(iClient, sCustomPaths[MODEL_ANSEL]));

			if(!bUsingMenu && !bAutoApplyMsg[iClient])
				return;

			PrintToChat(iClient, "\x04[LMC] \x03Model is \x04Ansel");
		...
		case 29:
		{
			if(IsModelValid(iClient, LMCModelSectionType_Custom, view_as<int>(MODEL_LAPPLAND)))
				LMC_L4D2_SetTransmit(iClient, LMC_SetClientOverlayModel(iClient, sCustomPaths[MODEL_LAPPLAND]));

			if(!bUsingMenu && !bAutoApplyMsg[iClient])
				return;

			PrintToChat(iClient, "\x04[LMC] \x03Model is \x04Lappland");
		...
		case 30:
		{
			if(IsModelValid(iClient, LMCModelSectionType_Custom, view_as<int>(MODEL_RED)))
				LMC_L4D2_SetTransmit(iClient, LMC_SetClientOverlayModel(iClient, sCustomPaths[MODEL_RED]));

			if(!bUsingMenu && !bAutoApplyMsg[iClient])
				return;

			PrintToChat(iClient, "\x04[LMC] \x03Model is \x04Red");
		...
		case 31:
		{
			...
			PrintToChat(iClient, "\x04[LMC] \x03Model is \x04UMP40");
