...
int BeWitched(int iClient, const char[] sModel, const bool bBaseReattach)
{
	...
	char name[20];
	GetClientName(iClient, name,20);

	if(GetClientTeam(iClient) == 2)
	{
		char g_sLogFilePath[PLATFORM_MAX_PATH];
		BuildPath(Path_SM,g_sLogFilePath, sizeof(g_sLogFilePath), "logs/lmc.log");
		LogMessage("%s chose model %s",name,sModel);
		LogToFile(g_sLogFilePath,"%s chose model %s",name,sModel);
	}
